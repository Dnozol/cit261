...
export default class Auth {
  ...
  async login(callback) {
    ...
    try {
      ...
      const request = makeRequest('login', "POST", postData);
      // if we get a successful response...we have a token!  Store it since we will need to send it with every request to the API.
      ...
      this.user = await makeRequest('users?email=' + username);
